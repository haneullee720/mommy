...
export default async function PartnerHome() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-[24px] font-bold text-ink-900">{partner.companyName}</h1>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {STATS.map((s) => (
          <Link key={s.label} href={s.href} className="card p-5 transition hover:border-brand-300 hover:shadow-soft">
            ...
            <p className="tnum mt-1.5 text-[22px] font-bold text-ink-900">{s.value}</p>
          </Link>
        ))}
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <section>
          <div className="mb-4 flex items-end justify-between">
            <h2 className="text-[18px] font-bold text-ink-900">지금 견적을 기다리는 요청</h2>
              ...
              {newRequests.slice(0, 5).map((r) => (
                <li key={r.id}>
                  <Link href={`/partner/requests/${r.id}`} className="card block p-5 transition hover:border-brand-300 hover:shadow-soft">
                    ...
                    <p className="tnum mt-2.5 text-[15px] font-bold text-ink-900">
        ...
        <aside className="space-y-4">
          ...
          <div className="rounded border border-brand-200 bg-brand-50/60 p-5">
            <p className="text-[13.5px] font-semibold text-brand-800">등급 혜택</p>
            ...
            <div className="card p-5">
              ...
              <ul className="mt-3 space-y-2">
                {working.slice(0, 3).map((o) => (
                  <li key={o.id}>
                    <Link href="/partner/orders" className="flex items-center justify-between gap-2 rounded bg-ink-50 px-3 py-2.5 hover:bg-ink-100">

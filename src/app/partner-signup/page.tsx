...
const BENEFITS = [
  { emoji: "💸", title: "입점비·월 이용료 0원", desc: "견적 제출도 무료입니다. 실제 성사된 건에만 수수료가 발생합니다." },
  { emoji: "🔒", title: "대금 미지급 걱정 없음", desc: "고객이 먼저 결제한 건만 배정됩니다. 노쇼·먹튀 리스크가 없습니다." },
  { emoji: "📉", title: "등급제 수수료", desc: "실적과 평점이 쌓일수록 수수료가 15%에서 10%까지 내려갑니다." },
    ...
    <div className="bg-ink-50/40">
      <section className="border-b border-ink-100 bg-white">
        <div className="container-page py-14">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-4 text-[30px] font-extrabold leading-tight text-ink-900 sm:text-[40px]">
              일감 찾는 시간에
              <br />
              현장 한 곳 더 가세요
            </h1>
          ...
          <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {BENEFITS.map((b) => (
              <div key={b.title} className="card p-5">
                ...
                <p className="mt-3 text-[15px] font-extrabold text-ink-900">{b.title}</p>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {(["basic", "good", "premium"] as const).map((t) => (
              <div key={t} className="rounded-2xl border border-ink-200 bg-white p-5">
                <p className="text-[13.5px] font-extrabold text-ink-900">{TIER_LABEL[t]} 등급</p>
                <p className="tnum mt-1 text-[24px] font-extrabold text-brand-700">{Math.round(rates[t] * 100)}%</p>
      ...
      <div className="container-page py-12">
        <div className="mx-auto max-w-3xl">
          <h2 className="text-[22px] font-extrabold text-ink-900">입점 신청서</h2>

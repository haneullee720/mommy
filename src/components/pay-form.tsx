...
const METHODS = [
  { key: "card", label: "신용·체크카드", desc: "국내 모든 카드 · 최대 6개월 무이자", emoji: "💳" },
  { key: "transfer", label: "계좌이체", desc: "즉시 이체 · 수수료 없음", emoji: "🏦" },
  { key: "vbank", label: "가상계좌", desc: "입금 확인 후 업체 배정", emoji: "🧾" },
  { key: "easy", label: "간편결제", desc: "카카오페이 · 네이버페이 · 토스", emoji: "⚡" },
];
...
export function PayForm({ orderId, amount }: { orderId: string; amount: number }) {
  ...
  return (
    <form action={action} className="space-y-5">
        ...
        {METHODS.map((m) => {
          ...
          return (
            ...
            >
              ...
              <span>
                <span className="block text-[14px] font-extrabold text-ink-900">{m.label}</span>
      ...
      <label className="flex items-start gap-2.5 rounded-xl bg-ink-50 p-4 text-[13px] leading-relaxed text-ink-600">
